...
def analyze_keypoints_in_file(sentences: List[Dict], keypoints: List[str], filename: str) -> List[Dict]:
    """
    Analyze keypoints and find most similar sentences within the same file using ROUGE-L
    """
    results = []

    print(f"  Analyzing {len(keypoints)} keypoints in {filename}...")

    for i, keypoint in enumerate(keypoints):
        # Calculate similarity scores for all sentences in this file
        similarities = []
...
def print_summary_per_file(all_results: Dict[str, List[Dict]]):
    ...
    """
    print("\n" + "=" * 80)
    print("KEYPOINT ANALYSIS SUMMARY (PER FILE)")
    print("=" * 80)
...
def main():
    ...
    for csv_file in csv_files:
        ...
        results = analyze_keypoints_in_file(sentences, keypoints, filename)
